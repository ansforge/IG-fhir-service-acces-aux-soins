...
A titre d'exemple, les codes d'erreur suivants pourront être envoyés du Hub vers la plateforme SAS : |statusCode | statusString | description | |——–|——–|——| | 102 | UNRECOGNIZED_MESSAGE_FORMAT | Le message n’a pas pu être désérialisé. | | 300 | INVALID_MESSAGE | Le message n’est pas conforme aux spécifications techniques (JSON Schema) | | 400 | EXPIRED_MESSAGE_BEFORE_ROUTING | Le message n’a pas été reçu par son destinataire, il a expiré sur le Hub avant de lui être délivré. | | 500 | DEAD_LETTERED_QUEUE | Le message n’a pas été reçu par son destinataire, il a expiré avant qu’il ne le dépile. |
